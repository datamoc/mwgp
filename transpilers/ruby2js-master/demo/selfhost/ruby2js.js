...
class $Range {constructor(begin, end, excludeEnd=false) {this.begin = begin; this.end = end; this.excludeEnd = excludeEnd} *[Symbol.iterator]() {let stop = this.excludeEnd ? this.end : this.end + 1; for (let i = this.begin; i < stop; i++) yield i}}
  ...
  class Converter extends Serializer {
    ...
    get convert() {
      ...
      if (this._need_range_class) {
        range_class = "class $Range {constructor(begin, end, excludeEnd=false) {this.begin = begin; this.end = end; this.excludeEnd = excludeEnd} *[Symbol.iterator]() {let stop = this.excludeEnd ? this.end : this.end + 1; for (let i = this.begin; i < stop; i++) yield i}}";
    ...
    get loose_break() {
      return this._loose_break
    };

    set loose_break(loose_break) {
      this._loose_break = loose_break
    };
...
`
    ...
    combine_properties(body) {
      for (let i = 0; i < body.length - 1; i++) {
        ...
        for (let j of new $Range(i + 1, body.length, true)) {
    ...
    on_break(n=null) {
      ...
      if (this._next_token === "return") {
        if (this._loose_break) return this.put("return");
    ...
    on_class(name, inheritance, ...body) {
      ...
      let convert_member = null;

      convert_member = (m) => {
        let sym, innerclass_name, sclass_body, cond, then_branch, else_branch, convert_branch;

        if (this._ast.type === "class_module" && m.type === "defs" && m.children.first?.type === "self") {
          m = m.updated("def", m.children.slice(1))
        };

        if (["def", "defm", "deff"].includes(m.type)) {
          if (m.children.first === "initialize" && !visible.initialize) {
            init = m;
            return null
          } else if (/=/.test(m.children.first)) {
            sym = `${(m.children.first ?? "").toString().slice(0, -1) ?? ""}`;
        ...
        } else if (["defs", "defp"].includes(m.type) && m.children.first?.type === "self") {
          if (/=$/m.test(m.children[1])) {
            return this.s(
              "prop",
              name,
            ...
            return this.s("prop", name, {[(m.children[1] ?? "").toString()]: {
              enumerable: this.s("true"),
              configurable: this.s("true"),
            ...
            return this.s("prototype", this.s(
              "send",
              name,
              `${m.children[1] ?? ""}=`,
          ...
          if (m.children[1] === "attr_accessor") {
            return m.children.slice(2).map((child_sym) => {
          ...
          } else if (["private", "protected", "public"].includes(m.children[1])) {
            return (() => { throw new Error(`class ${m.children[1] ?? ""} is not supported`, this._ast) })()
          } else {
            return this.s("send", name, ...m.children.slice(1))
          }
        ...
        } else if (["send", "block"].includes(m.type)) {
          return m
        } else if (m.type === "lvasgn") {
          return this.s(
            "send",
            name,
            `${m.children[0] ?? ""}=`,
        ...
        } else if (m.type === "send" && m.children[0].type === "cvar") {
          return this.s(
            "send",
            this.s("attr", name, `_${m.children[0].children[0].slice(2) ?? ""}`),
            ...m.children.slice(1)
          )
        } else if (m.type === "casgn" && m.children[0] == null) {
        ...
        } else if (this._ast.type === "class_module") {
          return m
          ...
          let [cond, then_branch, else_branch] = m.children;

          convert_branch = (branch) => {
            if (!branch) return null;
            let stmts = branch.type === "begin" ? branch.children : [branch];
            let converted = stmts.map(stmt => convert_member(stmt)).flat(Infinity).compact;

            return converted.length <= 1 ? converted.first : this.s(
              "begin",
              ...converted
            )
          };
          ...
          return this.s(
            "if",
            cond,
            convert_branch(then_branch),
            convert_branch(else_branch)
          )
        } else {
          return (() => { throw new Error(`class ${m.type ?? ""} not supported`, this._ast) })()
        }
      };

      body.splice(...[0, body.length].concat(body.map((m) => {
        let node = convert_member(m);
    ...
    on_sclass(target, body) {
      if (body?.type === "begin") body = body.children;
      if (!Array.isArray(body)) body = [body];

      let members = body.compact.map((member) => {
        ...
        } else if (member.type === "alias") {
          let [new_name, old_name] = member.children;

          return this.s(
            "send",
            target,
            `${new_name.children.first ?? ""}=`,
            this.s("send", target, old_name.children.first)
          )
        } else {
          return member
        }
      ...
      return this.parse(this.s("begin", ...members), "statement")
    };
    ...
    on_def(name, args, body=null) {
      let has_restarg, restarg, restarg_name, pop_stmt, style, nl;
      body ??= this.s("begin");
      ...
      if (add_implicit_block) {
        ...
        if (!has_implicit_block) {
          ...
          if (restarg) {
            ...
            body = body.type === "begin" ? this.s(
              "begin",
              pop_stmt,
              ...body.children
            ) : this.s("begin", pop_stmt, body)
          } else {
...
`);
  ...
  Converter._handlers.push("sclass");
  ...
  class Pipeline {
    ...
    get configure_converter() {
      ...
      this._converter.loose_break = this._options.loose_break || false;
