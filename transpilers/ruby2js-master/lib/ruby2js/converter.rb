...
module Ruby2JS
  ...
  class Converter < Serializer
    ...
    def convert
      ...
      if @need_range_class
        # Also implement the iterator protocol (ascending integers only, matching Ruby's
        # own Range#each): some conversions (e.g. `.each` over a range whose bounds aren't
        # compile-time literals) emit `for (x of new $Range(...))` rather than unrolling to
        # an index loop, and previously had no way to actually iterate the result.
...
require_relative 'converter/cast'
require_relative 'converter/case'
require_relative 'converter/case_match'
...
require_relative 'converter/jsraw'
...
require_relative 'converter/pnode'
require_relative 'converter/astro_file'
