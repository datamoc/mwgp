module Ruby2JS
  class Converter
    ...
    handle :class, :class_hash, :class_extend, :class_module do |name, inheritance, *body|
      ...
      # Named (rather than inline) and self-referential so a class-body-level conditional
      # (`alias foo bar unless method_defined?(:foo)` - common in RGSS "hotfix" scripts that
      # guard a reopened alias/def against being re-applied) can recursively convert whatever
      # statement(s) it guards through this exact same per-statement dispatch, rather than
      # needing its own separate, narrower copy of this logic.
      convert_member = nil
      convert_member = ->(m) do
        if \
          @ast.type == :class_module and m.type == :defs and
          m.children.first&.type == :self
        then
        ...
        if %i(def defm deff).include? m.type
        ...
        elsif %i(defs defp).include? m.type and m.children.first&.type == :self
          if m.children[1] =~ /=$/
          ...
          # A conditional class-body statement, e.g.
          #   if method_defined?(:foo)
          #     alias origFoo foo unless method_defined?(:origFoo)
          #     def foo(*args); ...; end
          #   end
          # - common in RGSS "hotfix" scripts guarding a reopened alias/def against being
          # re-applied. Convert whatever the branch guards the same way a bare class-body
          # statement would be, recursing into this same dispatch, then keep the guard as a
          # runtime `if` around the (possibly multi-statement) result.
          cond, then_branch, else_branch = m.children
          convert_branch = ->(branch) do
            next nil unless branch
            stmts = branch.type == :begin ? branch.children : [branch]
            converted = stmts.map { |stmt| convert_member.call(stmt) }.flatten.compact
            converted.length <= 1 ? converted.first : s(:begin, *converted)
          end
          s(:if, cond, convert_branch.call(then_branch), convert_branch.call(else_branch))
        else
    ...
    # Convert a top-level singleton class to assignments on its receiver.
    # Singleton classes nested in a regular class are handled by the class
    # handler above.
    handle :sclass do |target, body|
      body = body.children if body&.type == :begin
      body = [body] unless body.is_a?(Array)

      members = body.compact.map do |member|
        if member.type == :def
          s(:defs, target, member.children[0], member.children[1], member.children[2])
        elsif member.type == :alias
          new_name, old_name = member.children
          s(:send, target, "#{new_name.children.first}=",
            s(:send, target, old_name.children.first))
        else
          member
        end
      end
      parse s(:begin, *members), :statement
    end
  end
end
