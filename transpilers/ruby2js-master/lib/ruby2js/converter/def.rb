    ...
    handle :def, :defm, :async, :deff, :defget do |name, args, body=nil|
      ...
      if add_implicit_block
        ...
        unless has_implicit_block
          # JS requires a rest parameter to be last, so an implicit block param can't simply
          # be appended when one is present - same constraint the restarg/blockarg and
          # restarg/kwargs handling above this already works around, and the same fix: leave
          # the rest param as the true last formal parameter (so the caller-supplied block,
          # always passed as the trailing actual argument, lands inside it), and pop the
          # block back out of it as the method's first statement instead of naming it as its
          # own formal parameter.
          restarg = args.children.find { |a| a.type == :restarg }
          if restarg
            restarg_name = restarg.children.first || 'args'
            pop_stmt = s(:lvasgn, "_implicitBlockYield",
              s(:send, s(:lvar, restarg_name), :pop))
            body = body.type == :begin ? s(:begin, pop_stmt, *body.children) : s(:begin, pop_stmt, body)
          else
