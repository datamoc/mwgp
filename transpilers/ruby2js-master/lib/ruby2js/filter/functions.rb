...
module Ruby2JS
  module Filter
    module Functions
      ...
      def contains_plain_break?(node)
        return false unless Ruby2JS.ast_node?(node)
        return true if node.type == :break
        return false if [:block, :lambda].include?(node.type)
        node.children.any? { |child| contains_plain_break?(child) }
      end
      ...
      def on_send(node)
        ...
        # -> this.constructor.prototype[name] = block_var
          ...
          callback = args.first
          if args.length == 1 && [:block, :def].include?(callback&.type) &&
             contains_plain_break?(callback.children[2])
            callback_args = callback.children[1]
            variable = callback_args&.children&.first
            if variable&.type == :arg
              process S(:for_of,
                s(:lvasgn, variable.children.first), target, callback.children[2])
            else
      ...
      def on_block(node)
        ...
        if method == :each && node.children[1]&.children&.length == 1 &&
           contains_plain_break?(node.children[2])
          variable = node.children[1].children.first
          if variable&.type == :arg
            return process S(:for_of,
              s(:lvasgn, variable.children.first), call.children.first,
              node.children[2])
          end
        end
